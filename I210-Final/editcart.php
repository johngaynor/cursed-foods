...
//retrieve item id and make sure it is a numeric, positive value.
...
if (!filter_has_var(INPUT_GET, 'type')) {
    $error = "Edit action not found. Operation cannot proceed.";
    header("Location: error.php?m=$error");
    die();
}
...
if (array_key_exists($id, $cart)) {
    if ($action == 'increment') {
        ...
        $error = "Edit action is invalid. Operation cannot proceed.";
        header("Location: error.php?m=$error");
        die();
    }
}
